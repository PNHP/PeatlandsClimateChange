...
baseline <- parseMetadata("I:/climate_data/ClimateNA/historic/derived/normals/biovars/normals")
...
#         y="Precip of Driest Quarter (mm)") +
  ...
  p2 <- ggplot() +
    ...
    scale_color_manual(values = greens)  +
    scale_x_continuous(breaks=seq(floor(min(Wdf$temp)), ceiling(max(Wdf$temp)), 1)) +
    #scale_y_continuous(breaks=seq(floor(min(df$ppt)), ceiling(max(df$ppt)), 25)) +
        labs(x="Temp of Coldest Quarter (C)",
             y="Precip of Driest Quarter (mm)") +
