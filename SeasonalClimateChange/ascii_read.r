...
# precip
setwd(here("baseline","NA_NORM_6190_Monthly_ASCII"))
...
for (i in 1:length(gridlist)){
  ...
  fn <- paste(here("baseline","NA_NORM_6190_Monthly_ASCII_clip"),"/", names(gridlist[i]), ".asc", sep="")
  a <- crop(ras,bnd)
  writeRaster(a, filename = fn, format = "ascii", overwrite = TRUE) # datatype = "INT4S"
}

setwd(here("baseline","NA_NORM_6190_Monthly_ASCII_clip"))
raslist <- list.files(pattern="PPT")
gridlist <- as.list(raslist)
